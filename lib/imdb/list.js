...
var get_movies_page = function(url, callback, terminal) {
	...
	downcache(BASE + url, function(err, resp, body) {
		...
		if ($(pagination).html().split("&")[0] == "Next" && !terminal(movies)) {
...
module.exports = {
	...
	keyword: function(keyword, callback, terminal) {
		callback = callback || function(d) { console.log(d); }
		get_movies_page('/search/title?count=100&keywords='+keyword+'&start=1&title_type=feature', callback, terminal);
	}
